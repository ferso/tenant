...
var credentials   = typeof(connections.user) !=='undefined' && connections.user !=='undefined'  ? connections.user+':'+connections.password+'@' : '';
    ...
    createIndex : function(db){
          ...
          var collection = db.collection(that.collection);
           for( var i in this.attributes ){
             var index  = {};
            var unique = {};
            if( this.attributes[i].hasOwnProperty('index') === true){
                var index = ''+i+'';
            }
            if( this.attributes[i].hasOwnProperty('unique') === true){
                var unique =  { unique: true };
            }
            collection.ensureIndex(i,unique);
          }
    },
    updatedAt : function(){

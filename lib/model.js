...
const connections = require(fs.realpathSync('config/connections.js',{})).connections;

var credentials   = typeof(connections.default.user) && connections.default.user != '' ? connections.default.user+':'+connections.default.password+'@' : '';
const uri         = 'mongodb://'+credentials+connections.default.host+':'+connections.default.port+'/';
...
module.exports = {
    ...
    exec:function(callback){    
      ...
      var that = this;
      this.connect(function(db,_criteria,_collection,_data) {
          that._criteria   = _criteria;
          that._collection = _collection;
          that._data       = _data;
    ...
    connect:function(callback){   
      ...
      //open connection 
      db.open(function(err, db, _criteria, _collection, _data){
        if(!err){
           // Display output in testing mode
           //--------------------------------------------------       
          //console.log("Connected to database server:", connections.default.host);
          callback(err||db,_criteria,_collection,_data);
        }
      }.bindThis(this._criteria,this._collection,this._data));
    ...
    findById : function(id,fields,options){
     ...
     create : function(data,callback){
        console.log('create collection: ',this._collection);
        this._data        = data
    ...
    *
    ...
    _find : function(db,callback){
      try{
        ...
        collection = db.collection(this._collection);
    ...
    _create : function(db,callback){
      ...
      // console.log('collection',that._collection);
        var collection = db.collection(this._collection);
    ...
    _update : function(db,callback){
        ...
        try{
         ...
         var collection = db.collection(this._collection);
             collection.update(this._criteria,this._set,function(e,r) {              
                if(!e){
      ...
      try{           
          this.connect(function(db){
              if(db){
                ...
                var collection = db.collection(that._collection);

...
// ----------------------------------------------
fs.readdirSync(fs.realpathSync('services',{})).forEach(function(file,i) {
    var name            = file.replace('.js','') ;
    var service_name    = name.charAt(0).toUpperCase() + name.slice(1);
    this[service_name]  = require(join(fs.realpathSync('services',{}),'/', file));
});
